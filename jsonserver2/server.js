...
server.get('/api/boards', (req, res) => {
  ...
  const boards = db.get('boards')
    ...
    .map(board => ({
      ...
      is_starred: board.isStarred || false
    }));
...
// Получение последних 3 досок пользователя
server.get('/api/recent', (req, res) => {
  const db = router.db;
  const userId = req.user.id;
  console.log('User ID for recent boards:', userId);
  
  try {
    // Находим доски, к которым у пользователя есть доступ через board_users
    const userBoardIds = db.get('board_users')
      .filter({ user_id: userId })
      .map('board_id')
      .value();
    
    console.log('User board IDs:', userBoardIds);
    
    // Если у пользователя нет досок в board_users, возвращаем пустой массив
    if (!userBoardIds || userBoardIds.length === 0) {
      return res.json({
        boards: []
      });
    }
    
    // Получаем последние 3 доски, отсортированные по дате обновления
    const recentBoards = db.get('boards')
      .filter(board => userBoardIds.includes(board.id))
      .orderBy('updated_at', 'desc')
      .take(3)
      .value();
    
    console.log('Recent boards found:', recentBoards.length);
    ...
    res.json({
      boards: recentBoards.map(board => ({
        ...
        is_starred: board.isStarred || false  // ← исправлено здесь
      }))
    });
  } catch (error) {
    console.error('Error in /api/boards/recent:', error);
    res.status(500).json({ message: 'Ошибка сервера' });
  }
});

// Получение избранных досок пользователя
// Получение избранных досок пользователя
server.get('/api/starred', (req, res) => {
  const db = router.db;
  const userId = req.user.id;
  console.log(userId);
  ...
  try {
    // Находим доски, к которым у пользователя есть доступ
    const userBoardIds = db.get('board_users')
      .filter({ user_id: userId })
      .map('board_id')
      .value();
    
    console.log('User board IDs for starred:', userBoardIds);
    
    // Если у пользователя нет досок, возвращаем пустой массив
    if (!userBoardIds || userBoardIds.length === 0) {
      return res.json({
        boards: []
      });
    ...
    // Получаем избранные доски - ИСПРАВЛЕНО: используем isStarred вместо is_starred
    const starredBoards = db.get('boards')
      .filter(board => userBoardIds.includes(board.id) && (board.isStarred === true))
      .orderBy('updated_at', 'desc')
      .value();
    
    console.log('Starred boards found:', starredBoards.length);
    
    res.json({
      boards: starredBoards.map(board => ({
        id: board.id,
        title: board.title,
        updated_at: board.updated_at || new Date().toISOString(),
        created_at: board.created_at || new Date().toISOString(),
        is_starred: board.isStarred || false  // ← тоже исправлено
      }))
    });
  } catch (error) {
    console.error('Error in /api/boards/starred:', error);
    res.status(500).json({ message: 'Ошибка сервера' });
  }
});
